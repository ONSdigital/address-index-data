...
ONS <- read.table(paste0('//tdata8/AddressIndex/Beta_Results/HMRC/EduBase_Schools_July_2017_01_response.csv'), header=T, sep=',',  quote = "\"", stringsAsFactors=F)
HMRC <-  read.table(paste0('//tdata8/AddressIndex/Beta_Results/HMRC/UPRN_SCHOOLS.txt'), header=T, sep='|',  quote = "\"", stringsAsFactors=F)
...
matched_groups <- joined %>% group_by(id) %>% summarise(compare_status = eval_match(UPRN_ons, UPRN_hmrc, STATUS_ons, STATUS_hmrc)) %>%
  group_by(id) %>%  mutate(compare_simple = simplify_eval(compare_status) ) %>% ungroup()
matched_groups$compare_simple <- factor(matched_groups$compare_simple, 
...
t(t(table(matched_groups$compare_simple)))
####################################
...
joined <- joined %>% group_by(id) %>% 
  ...
  ungroup()
joined$compare_simple <- factor(joined$compare_simple, levels = c('Top - single agreement', 'Top - multiple agreement', 'Top - parent agreement', 'In set - single (parent) agreement', 
                                                                  'In set - multiple (parent) agreement', 'disagreement', 'ons_only (top)', 'ons_only (set)', 'hmrc_only', 'both_missing'))
matched_groups2 <- joined %>% group_by(id) %>% summarise(compare_simple=compare_simple[1])
t(t(table(matched_groups2$compare_simple)))    # looks the same as above, hurray 
...
# export some subset for clerical resolutions ...
...
write.csv(disagree, file= 'got_worse/EduBase_Schools_UPRN_disagree.csv', row.names = F)
