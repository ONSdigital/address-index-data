...
trait HybridAddress {
  ...
  def rowToLpi(row: Row): Map[String, Any] = Map(
    ...
    "location" -> (if(row.isNullAt(3)) 0.0 else row.get(3)),
    "easting" -> (if (row.isNullAt(4)) null else row.getFloat(4)),
    "northing" -> (if (row.isNullAt(5)) null else row.getFloat(5)),
    "parentUprn" -> (if (row.isNullAt(6)) null else row.getLong(6)),
    "multiOccCount" -> (if (row.isNullAt(7)) 0.0 else row.getShort(7)),
    "blpuLogicalStatus" -> (if (row.isNullAt(8)) null else row.getByte(8)),
    "localCustodianCode" -> (if (row.isNullAt(9)) null else row.getShort(9)),
    "rpc" -> (if (row.isNullAt(10)) null else row.getByte(10)),
    "organisation" -> (if(row.isNullAt(11)) null else normalize(row.getString(11))),
    ...
    "lpiLogicalStatus" -> (if (row.isNullAt(27)) null else row.getByte(27)),
    "usrnMatchIndicator" -> (if (row.isNullAt(28)) null else row.getByte(28)),
    "language" -> (if (row.isNullAt(29)) null else row.getString(29)),
    "streetDescriptor" -> (if (row.isNullAt(30)) null else normalize(row.getString(30))),
    "townName" -> (if (row.isNullAt(31)) null else normalizeTowns(row.getString(31))),
    "locality" -> (if (row.isNullAt(32)) null else normalize(row.getString(32))),
    "streetClassification" -> (if (row.isNullAt(33)) null else row.getByte(33)),
    "lpiStartDate" -> (if (row.isNullAt(34)) null else row.getDate(34)),
    "lpiLastUpdateDate" -> (if (row.isNullAt(35)) null else row.getDate(35)),
    "lpiEndDate" -> (if (row.isNullAt(36)) null else row.getDate(36)),
  ...
  def rowToPaf(row: Row): Map[String, Any] = Map(
    ...
    "uprn" -> row.getLong(3),
    "udprn" -> (if (row.isNullAt(4)) null else row.getInt(4)),
