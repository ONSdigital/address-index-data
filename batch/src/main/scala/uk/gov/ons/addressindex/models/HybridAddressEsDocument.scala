...
object HybridAddressEsDocument extends EsDocument {
  ...
  def rowToLpi(row: Row): Map[String, Any] = Map(
    ...
    "rpc" -> row.getByte(10),
    "organisation" -> splitAndCapitaliseTowns(row.getString(11)),
    "legalName" -> row.getString(12),
    "usrn" -> row.getInt(13),
    "lpiKey" -> row.getString(14),
    "paoText" -> splitAndCapitaliseTowns(row.getString(15)),
    "paoStartNumber" -> (if (row.isNullAt(16)) null else row.getShort(16)),
    "paoStartSuffix" -> splitAndCapitaliseTowns(row.getString(17)),
    "paoEndNumber" -> (if (row.isNullAt(18)) null else row.getShort(18)),
    "paoEndSuffix" -> splitAndCapitaliseTowns(row.getString(19)),
    "saoText" -> splitAndCapitaliseTowns(row.getString(20)),
    "saoStartNumber" -> (if (row.isNullAt(21)) null else row.getShort(21)),
    "saoStartSuffix" -> splitAndCapitaliseTowns(row.getString(22)),
    "saoEndNumber" -> (if (row.isNullAt(23)) null else row.getShort(23)),
    "saoEndSuffix" -> splitAndCapitaliseTowns(row.getString(24)),
    ...
    "locality" -> splitAndCapitaliseTowns(row.getString(32)),
    ...
    "mixedNag" -> generateFormattedNagAddress(
      if (row.isNullAt(21)) "" else row.getShort(21).toString,
      splitAndCapitaliseTowns(row.getString(22)),
      if (row.isNullAt(23)) "" else row.getShort(23).toString,
      splitAndCapitaliseTowns(row.getString(24)),
      splitAndCapitaliseTowns(row.getString(20)),
      splitAndCapitaliseTowns(row.getString(11)),
      if (row.isNullAt(16)) "" else row.getShort(16).toString,
      row.getString(17),
      if (row.isNullAt(18)) "" else row.getShort(18).toString,
      splitAndCapitaliseTowns(row.getString(19)),
  ...
  def rowToPaf(row: Row): Map[String, Any] = Map(
    ...
    "uprn" -> row.getLong(3),
    "udprn" -> row.getInt(4),
    "organisationName" -> splitAndCapitaliseTowns(Option(row.getString(5)).getOrElse("")),
    "departmentName" -> splitAndCapitaliseTowns(Option(row.getString(6)).getOrElse("")),
    "subBuildingName" -> splitAndCapitaliseTowns(Option(row.getString(7)).getOrElse("")),
    "buildingName" -> splitAndCapitaliseTowns(Option(row.getString(8)).getOrElse("")),
    "buildingNumber" -> (if (row.isNullAt(9)) null else row.getShort(9)),
    "dependentThoroughfare" -> splitAndCapitaliseTowns(Option(row.getString(10)).getOrElse("")),
    "thoroughfare" -> splitAndCapitalise(Option(row.getString(11)).getOrElse("")),
    "doubleDependentLocality" -> splitAndCapitaliseTowns(Option(row.getString(12)).getOrElse("")),
    ...
    "welshDependentThoroughfare" -> splitAndCapitaliseTowns(Option(row.getString(18)).getOrElse("")),
    "welshThoroughfare" -> splitAndCapitaliseTowns(Option(row.getString(19)).getOrElse("")),
    "welshDoubleDependentLocality" -> splitAndCapitaliseTowns(Option(row.getString(20)).getOrElse("")),
    "welshDependentLocality" -> splitAndCapitaliseTowns(Option(row.getString(21)).getOrElse("")),
