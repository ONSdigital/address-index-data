...
object HybridAddressNisraEsDocument extends EsDocument {
  ...
  def rowToLpi(row: Row): Map[String, Any] = Map(
    ...
    "secondarySort" -> addLeadingZeros(row.getString(15) + " " + (if (row.isNullAt(21)) "" else row.getShort(21).toString) + row.getString(22) + " " + row.getString(11) + " " + row.getString(20))
  )
  ...
  def rowToNisra(row: Row): Map[String, Any] = {
    ...
    Map(
      ...
      "secondarySort" -> addLeadingZeros(Option(row.getString(8)).getOrElse("") + " " + Option(row.getString(9)).getOrElse("") + Option(row.getString(11)).getOrElse("") + " " + Option(row.getString(13)).getOrElse("") + " " + Option(row.getString(15)).getOrElse(""))
    )
  }
